...
def test_output_to_file(mock_repositories, tmp_path):
    ...
    # Patch sys.argv
    with patch.object(sys, 'argv', ['vendetect', test_repo, source_repo, '--format', 'csv', '--output', str(output_file)]):
        try:
            main()
...
def test_output_file_exists_with_force(mock_repositories, tmp_path):
    ...
    # Patch sys.argv
    with patch.object(sys, 'argv', ['vendetect', test_repo, source_repo, '--format', 'json', '--output', str(output_file), '--force']):
        try:
            main()
